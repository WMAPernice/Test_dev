...
class Statistics:
    ...
    def per_class(zipped: zip, norm_value=65536, save_name='', inspect = False) -> dict:
        ...
        for t in zipped: # zipped is the entire folder structure?
            class_images = [] # bug fixed <- class image list reset
            for class_dir in t: # t is a tuple
                class_name = class_dir.name
                # read from each dir and append to the images
                for file in class_dir.iterdir():
                    image = tiff.imread(str(file))
                    class_images.append(image) # <- appends images in the folder for each class
                ...
                stats[class_name] = (mean, stdev) # dictionary with mean and std for each class
    ...
    @staticmethod
    def one_dataset(directory:[Path], norm_value=65536, save_name='') -> tuple:

        _dirs = Path(directory)
        print(_dirs)
        #print(len(_dirs))

        images = [] #test
        for file in _dirs.iterdir():
            if ".tif" in str(file):
                image = tiff.imread(str(file))
                #print(image.shape)
                images.append(image)
